...
- Form scripts can print: `api.pdf({ html, name, width, table, record_id })` inside an action rasterises the html to an A4 PDF, paginating tall documents, uploads it as a `file`, and returns the stored file so the script can write its id into a `file` field. The DOM is rasterised rather than typeset, which is what lets a generated document carry any script the browser can draw without shipping a font with the app; jsPDF and html2canvas load on demand.
